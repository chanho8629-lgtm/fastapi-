...
- 입력 데이터: 작품 메타정보, 경매 상태값, 가격/입찰 이력, 이미지 분석 결과
- LLM 생성 항목: 가격 매력도 요약, 리스크 요인, 예상 낙찰 시나리오, 입찰 전략 제안
- REG(회귀) 연계: 조회수 회귀 예측값을 LLM 프롬프트 컨텍스트에 함께 넣어 종합 판단에 활용
- 응답 형태: 빠른 요약(`fast`)과 근거 포함 상세 리포트(`hybrid RAG`)를 분리 제공
- 운영 포인트: 응답시간, 리포트 채택률, 재질문율을 함께 모니터링해 프롬프트/검색 품질 개선

### FER-anything 설명

- 역할: 문서/이미지에서 특징(feature)과 핵심 문맥을 추출해 LLM 분석 입력을 정리하는 전처리 계층
- 입력: 작품 이미지, 경매 리포트 문서, 메타데이터(가격/입찰/반응 지표)
- 처리: 노이즈 제거, 핵심 키워드/수치 추출, 분석에 필요한 구조화 컨텍스트 생성
- 출력: LLM/RAG가 바로 사용할 수 있는 요약 컨텍스트 + 근거 포인트
- 기대 효과: LLM 응답 일관성 향상, 불필요 토큰 감소, 근거 기반 설명력 강화
